<template>
  <div class="space-y-6">
    ...
    <!-- Abas: select no mobile, botões no desktop -->
    <select
      v-model="abaAtiva"
      class="sm:hidden w-full text-sm font-medium bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-lg px-3 py-2 border-0 cursor-pointer"
    >
      <option v-for="aba in abas" :key="aba.key" :value="aba.key">{{ aba.label }}</option>
    </select>
    <div class="hidden sm:flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg p-0.5 w-fit">
      <button
        v-for="aba in abas"
        :key="aba.key"
        class="px-4 py-1.5 text-sm font-medium rounded-md transition-all cursor-pointer whitespace-nowrap"
