  ...
  <div class="space-y-4">
    ...
    <UFormField v-if="tipo !== 'transferencia'" label="Tipo">
      <div class="flex gap-2">
        <button
          ...
          :key="opt.value"
          type="button"
          class="flex-1 flex items-center gap-2 p-2.5 rounded-lg border-2 transition-colors"
    ...
    <div class="flex justify-end gap-2 pt-1">
      ...
      <UButton color="primary" :loading="loading" @click="handleSubmit">
        {{ tipo === 'receita' ? 'Adicionar receita' : tipo === 'despesa' ? 'Adicionar despesa' : 'Transferir' }}
      </UButton>
    </div>
  </div>
</template>
