...
export default defineEventHandler(async (event) => {
  ...
  db.prepare(
    `UPDATE cartoes
     SET nome = ?, banco = ?, banco_key = ?, ultimos_digitos = ?, limite = ?, melhor_data_compra = ?, vencimento = ?, cor = ?
     WHERE id = ?`
  ).run([body.nome.trim(), body.banco.trim(), body.banco_key?.trim() ?? '', ultimosDigitos, body.limite, body.melhor_data_compra, body.vencimento, body.cor ?? null, id])
